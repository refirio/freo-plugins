...
/* メイン処理 */
function freo_display_entry_image()
{
	global $freo;

	if ($freo->config['plugin']['entry_image']['display_limit'] == 0) {
		return;
	}
	...
	//エントリー取得
	$stmt = $freo->pdo->prepare('SELECT * FROM ' . FREO_DATABASE_PREFIX . 'entries WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND (close IS NULL OR close >= :now2) AND image IS NOT NULL AND display = \'publish\' ' . $condition . ' ORDER BY datetime DESC LIMIT :limit');
	$stmt->bindValue(':now1',  date('Y-m-d H:i:s'));
	$stmt->bindValue(':now2',  date('Y-m-d H:i:s'));
	$stmt->bindValue(':limit', intval($freo->config['plugin']['entry_image']['display_limit']), PDO::PARAM_INT);
	$flag = $stmt->execute();
	if (!$flag) {
	...
	$freo->smarty->assign(array(
		'plugin_entry_images'           => $entries,
		'plugin_entry_image_associates' => $entry_associates,
		'plugin_entry_image_filters'    => $entry_filters,
		'plugin_entry_image_securities' => $entry_securities,
		'plugin_entry_image_tags'       => $entry_tags,
		'plugin_entry_image_files'      => $entry_files,
		'plugin_entry_image_thumbnails' => $entry_thumbnails,
		'plugin_entry_image_images'     => $entry_images,
		'plugin_entry_image_texts'      => $entry_texts
	));
