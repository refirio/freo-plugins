...
/* メイン処理 */
function freo_page_entry_image()
{
	global $freo;
	...
	if (!isset($_GET['category'])) {
		if (!$freo->config['plugin']['entry_image']['display'] and $condition == null) {
	...
	//表示順設定
	if ($freo->config['plugin']['entry_image']['order'] == 'datetime_desc') {
	...
	//表示件数設定
	if ($freo->config['plugin']['entry_image']['limit']) {
		$limit = 'LIMIT ' . (intval($freo->config['plugin']['entry_image']['default_limit']) * ($_GET['page'] - 1)) . ', ' . intval($freo->config['plugin']['entry_image']['default_limit']);
	} else {
		$limit = null;
	}
	...
	//エントリー数・ページ数取得
	if (isset($_GET['category'])) {
		$stmt = $freo->pdo->prepare('SELECT COUNT(*) FROM ' . FREO_DATABASE_PREFIX . 'entries LEFT JOIN ' . FREO_DATABASE_PREFIX . 'category_sets ON id = entry_id WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND (close IS NULL OR close >= :now2) AND image IS NOT NULL AND category_id = :category ' . $condition);
		$stmt->bindValue(':now1',     date('Y-m-d H:i:s'));
		$stmt->bindValue(':now2',     date('Y-m-d H:i:s'));
		$stmt->bindValue(':category', $_GET['category']);
	} else {
		$stmt = $freo->pdo->prepare('SELECT COUNT(*) FROM ' . FREO_DATABASE_PREFIX . 'entries WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND (close IS NULL OR close >= :now2) AND image IS NOT NULL ' . $condition);
		$stmt->bindValue(':now1', date('Y-m-d H:i:s'));
		$stmt->bindValue(':now2', date('Y-m-d H:i:s'));
	}
	$flag = $stmt->execute();
	...
	$entry_count = $data[0];
	$entry_page  = $freo->config['plugin']['entry_image']['limit'] ? ceil($entry_count / $freo->config['plugin']['entry_image']['default_limit']) : 1;
