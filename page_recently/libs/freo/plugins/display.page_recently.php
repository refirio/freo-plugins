...
/* メイン処理 */
function freo_display_page_recently()
{
	global $freo;
	...
	//ページ取得
	$stmt = $freo->pdo->prepare('SELECT * FROM ' . FREO_DATABASE_PREFIX . 'pages WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND display = \'publish\' AND (close IS NULL OR close >= :now2) ' . $condition . ' ORDER BY datetime DESC LIMIT :limit');
	$stmt->bindValue(':now1',  date('Y-m-d H:i:s'));
	$stmt->bindValue(':now2',  date('Y-m-d H:i:s'));
	$stmt->bindValue(':limit', intval($freo->config['plugin']['page_recently']['default_limit']), PDO::PARAM_INT);
	$flag = $stmt->execute();
	if (!$flag) {
	...
	$freo->smarty->assign(array(
		'plugin_page_recentries'          => $pages,
		'plugin_page_recently_associates' => $page_associates,
		'plugin_page_recently_filters'    => $page_filters,
		'plugin_page_recently_securities' => $page_securities,
		'plugin_page_recently_tags'       => $page_tags,
		'plugin_page_recently_files'      => $page_files,
		'plugin_page_recently_thumbnails' => $page_thumbnails,
		'plugin_page_recently_images'     => $page_images,
		'plugin_page_recently_texts'      => $page_texts
	));
