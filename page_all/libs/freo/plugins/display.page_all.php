...
/* メイン処理 */
function freo_display_page_all()
{
	global $freo;
	...
	//ページ取得
	$stmt = $freo->pdo->prepare('SELECT * FROM ' . FREO_DATABASE_PREFIX . 'pages WHERE approved = \'yes\' AND (status = \'publish\' OR (status = \'future\' AND datetime <= :now1)) AND (close IS NULL OR close >= :now2) ' . $condition . ' ORDER BY pid, sort, id');
	...
	$freo->smarty->assign(array(
		'page_alls'           => $pages,
		'page_all_associates' => $page_associates,
		'page_all_filters'    => $page_filters,
		'page_all_securities' => $page_securities,
		'page_all_tags'       => $page_tags,
		'page_all_files'      => $page_files,
		'page_all_thumbnails' => $page_thumbnails,
		'page_all_images'     => $page_images
	));
